@props(['subitems', 'text', 'tooltip', 'dropdownFooter'])

@php
    $isDropdown = isset($subitems);
    $hasTooltip = isset($tooltip);
    $itemClasses = 'mr-1 rounded-lg p-2 text-gray-500 hover:bg-gray-100 hover:text-gray-900 focus:ring-4 focus:ring-gray-300 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-600';
@endphp
...
@if($isDropdown)
    ...
    <div id="dropdown-{{ $attributes->get('id') }}" class="z-50 my-4 hidden max-w-sm list-none divide-y divide-gray-100 overflow-hidden rounded bg-white text-base shadow-lg dark:divide-gray-600 dark:bg-gray-700">
...
<!-- Apps -->
<button type="button" data-dropdown-toggle="apps-dropdown" class="{{ $itemClasses }}">
...
{{--
<!-- Theme toggle -->
<button id="theme-toggle" data-tooltip-target="tooltip-toggle" type="button" class="{{ $itemClasses }}">
